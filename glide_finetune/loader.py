...
def random_resized_crop(image, shape, resize_ratio=1.0):
    ...
    """
    image_transform = transforms.RandomResizedCrop(shape, scale=(resize_ratio, 1.0), ratio=(1.0, 1.0))
...
class TextImageDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        try:
            original_pil_image = Image.open(image_file).convert("RGB")
        except (OSError, ValueError) as e:
            print(f"An exception occurred trying to load file {image_file}.")
            print(f"Skipping index {ind}")
            return self.skip_sample(ind)
        if self.enable_upsample:  # the base image used should be derived from the cropped high-resolution image.
            upsample_pil_image = random_resized_crop(original_pil_image, (
            self.side_x * self.upscale_factor, self.side_y * self.upscale_factor), resize_ratio=self.resize_ratio)
            upsample_tensor = pil_image_to_norm_tensor(upsample_pil_image)
            base_pil_image = upsample_pil_image.resize((self.side_x, self.side_y), resample=Image.BICUBIC)
            base_tensor = pil_image_to_norm_tensor(base_pil_image)
            # return torch.tensor(tokens), torch.tensor(mask, dtype=torch.bool), base_tensor, upsample_tensor
            return tokens, mask, base_tensor, upsample_tensor

        base_pil_image = random_resized_crop(original_pil_image, (self.side_x, self.side_y),
                                             resize_ratio=self.resize_ratio)
        base_tensor = pil_image_to_norm_tensor(base_pil_image)
        # return torch.tensor(tokens), torch.tensor(mask, dtype=torch.bool), base_tensor
