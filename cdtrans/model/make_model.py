...
from cdtrans.model.backbones.resnet import ResNet, Bottleneck
from cdtrans.model.backbones.resnet_ibn_a import resnet50_ibn_a, resnet101_ibn_a
from cdtrans.model.backbones.se_resnet_ibn_a import se_resnet101_ibn_a
from cdtrans.model.backbones.vit_pytorch import vit_base_patch16_224_TransReID, vit_small_patch16_224_TransReID
from cdtrans.model.backbones.vit_pytorch_uda import uda_vit_base_patch16_224_TransReID, \
    uda_vit_small_patch16_224_TransReID

from cdtrans.loss.arcface import ArcFace
from cdtrans.loss.metric_learning import Arcface, Cosface, AMSoftmax, CircleLoss
...
class Backbone(nn.Module):
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None,
        ...
        elif self.task_type == 'classify_DA':  # test for classify domain adaptation
            if self.cos_layer:
                cls_score = self.arcface(feat, label)
            else:
                cls_score = self.classifier(feat)
...
class TransformerBuilder(nn.Module):
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg, model_dict):
        ...
        if cfg.MODEL.TASK_TYPE == 'classify_DA':
            self.base = model_dict[cfg.MODEL.Transformer_TYPE](
                img_size=cfg.INPUT.SIZE_CROP, aie_xishu=cfg.MODEL.AIE_COE,
                local_feature=cfg.MODEL.LOCAL_F,
                stride_size=cfg.MODEL.STRIDE_SIZE,
                drop_path_rate=cfg.MODEL.DROP_PATH)
        else:
            self.base = model_dict[cfg.MODEL.Transformer_TYPE](
                img_size=cfg.INPUT.SIZE_TRAIN, aie_xishu=cfg.MODEL.AIE_COE,
                local_feature=cfg.MODEL.LOCAL_F,
                stride_size=cfg.MODEL.STRIDE_SIZE,
                drop_path_rate=cfg.MODEL.DROP_PATH)
    ...
    def _load_parameter(self, pretrain_choice, model_path):
        if pretrain_choice == 'imagenet':
            ...
            print('Do NOT load pretrained model. Train from scratch: TransformerBuilder')
...
class UdaTransformerBuilder(nn.Module):

    def __init__(self, num_classes, camera_num, view_num, cfg, model_dict):
        super(UdaTransformerBuilder, self).__init__()
        ...
        if pretrain_choice == 'imagenet':
            ...
            print('Do NOT load pretrained model. Train from scratch: UdaTransformerBuilder')
    ...
    def load_param_finetune(self, model_path):
        if not os.path.isfile(model_path):
            print(f"load_param_finetune can NOT find model_path: {model_path}")
            return
...
def make_model(cfg, num_class, camera_num, view_num):
    if cfg.MODEL.NAME == 'transformer':
        if cfg.MODEL.BLOCK_PATTERN == '3_branches':
            model = UdaTransformerBuilder(num_class, camera_num, view_num, cfg, __model_dict)
            print('===========building UDA Transformer===========')
        else:
            model = TransformerBuilder(num_class, camera_num, view_num, cfg, __model_dict)
