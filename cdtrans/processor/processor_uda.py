...
from cdtrans.datasets.bases import ImageDataset
from cdtrans.datasets.make_dataloader import source_target_train_collate_fn
from cdtrans.utils.meter import AverageMeter
from cdtrans.utils.metrics import R1_mAP_eval, R1_mAP_draw_figure, Class_accuracy_eval
...
def update_feat(cfg, epoch, model, train_loader1, train_loader2, device,
                feat_memory1, feat_memory2, label_memory1, label_memory2):
    model.eval()
...
def count_target_usage(logger, idxs, label_memory1, label_memory2, img_num1, img_num2, source_idxs=None):
    ...
    per_class_select_num = torch.bincount(label_memory2[unique_knnidx])  # TODO: there might be a bug, TypeError
...
def do_train_uda(cfg,
    ...
    if '384' in cfg.MODEL.Transformer_TYPE or 'small' in cfg.MODEL.Transformer_TYPE:  # small model
    ...
    # train
    # epochs, eval_period, log_period, checkpoint_period = 3, 1, 1, 1  # just for debugging
    for epoch in range(1, epochs + 1):
        ...
        if (epoch - 1) % update_epoch == 0:
            feat_memory1, feat_memory2, label_memory1, label_memory2 = update_feat(
                cfg, epoch, model, train_loader1, train_loader2, device,
                feat_memory1, feat_memory2, label_memory1, label_memory2)
            dynamic_top = 1
            print('source and target topk==', dynamic_top)

            target_label, knnidx, knnidx_topk, target_knnidx = compute_knn_idx(
                logger, model, train_loader1, train_loader2,
                feat_memory1, feat_memory2, label_memory1, label_memory2,
                img_num1, img_num2, topk=dynamic_top, reliable_threshold=0.0)
            del train_loader

            train_loader = generate_new_dataset(
                cfg, logger, label_memory2, s_dataset, t_dataset, knnidx,
                target_knnidx, target_label, label_memory1, img_num1, img_num2,
        ...
        for n_iter, (imgs, vid, target_cam, target_view, file_name, idx) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):  # Automatic Mixed Precision
                ...
                outputs = model(img, t_img, target, cam_label=target_cam, view_label=target_view)
                (self_score1, self_feat1, _), (score2, feat2, _), (score_fusion, _, _), aux_data = outputs
            ...
            if (n_iter + 1) % log_period == 0:
                logger.info(
                    "Epoch[{}] Iteration[{}/{}] Loss1: {:.3f}, Loss2: {:.3f}, Loss3: {:.3f},  "
                    "Acc: {:.3f}, Acc2: {:.3f}, Acc2_pse: {:.3f}, Base Lr: {:.2e}"
        ...
        end_time = time.time()
        time_per_batch = (end_time - start_time) / (last_n_iter + 1)
        if cfg.MODEL.DIST_TRAIN:
            if dist.get_rank() == 0:
        ...
        if epoch % checkpoint_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    torch.save(model.state_dict(), os.path.join(cfg.OUTPUT_DIR, f"{cfg.MODEL.NAME}_{epoch}.pth"))
            else:
                torch.save(model.state_dict(), os.path.join(cfg.OUTPUT_DIR, f"{cfg.MODEL.NAME}_{epoch}.pth"))
            ...
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (img, vid, camid, camids, target_view, idex) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        camids = camids.to(device)
                        target_view = target_view.to(device)
                        output_probs = model(img, img, cam_label=camids, view_label=target_view,
                                             return_logits=True, cls_embed_specific=False)
                ...
                if accuracy > best_model_mAP:
                    ...
                    torch.save(model.state_dict(), os.path.join(cfg.OUTPUT_DIR, f"{cfg.MODEL.NAME}_best_model.pth"))
                logger.info("Classify Domain Adaptation Validation Results - Epoch: {}".format(epoch))
                logger.info(
    ...
    # inference
    print('best model performance is {}'.format(best_model_mAP))
    ...
    if cfg.MODEL.TASK_TYPE == 'classify_DA':
        ...
        logger.info("Classify Domain Adaptation Validation Results - Best model")
...
def do_inference_uda(cfg, model, val_loader, num_query):
    ...
    if cfg.TEST.EVAL:
        if cfg.MODEL.TASK_TYPE == 'classify_DA':
            accuracy, mean_ent = evaluator.compute()
            logger.info("Classify Domain Adaptation Validation Results - In the source trained model")
            logger.info("Accuracy: {:.1%}".format(accuracy))
            return
        else:
            cmc, mAP, _, _, _, _, _ = evaluator.compute()
