...
class OurAPI(BaseImageDataset):
    ...
    def __init__(self, cfg, root_train='./datasets/Corrected_Market1501', root_val='./datasets/Corrected_Market1501',
                 verbose=True, syn_num: int = 0, **kwargs):
        super(OurAPI, self).__init__()

        self.cfg = cfg
        self.syn_num = syn_num

        self.train_dir = os.path.join(root_train, 'trainval')
        self.query_dir = os.path.join(root_val, 'test_probe')
        self.gallery_dir = os.path.join(root_val, 'test_gallery')
