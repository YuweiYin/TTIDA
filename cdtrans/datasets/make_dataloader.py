...
from cdtrans.datasets.preprocessing import RandomErasing
from cdtrans.datasets.sampler import RandomIdentitySampler
from cdtrans.datasets.sampler_ddp import RandomIdentitySampler_DDP
from cdtrans.datasets.ourapi import OurAPI
from cdtrans.datasets.office_31 import Office31
from cdtrans.datasets.office_home import OfficeHome
from cdtrans.datasets.domainnet import DomainNet
from cdtrans.datasets.visda import VisDA
...
def train_collate_fn(batch):
    """
    the input of collate_fn is a list, whose length equals to batch_size. each item in the list comes from __getitem__
    """
...
def make_dataloader(cfg):
    ...
    dataset = __dataset_dict[cfg.DATASETS.NAMES](
        cfg, root_train=cfg.DATASETS.ROOT_TRAIN_DIR, root_val=cfg.DATASETS.ROOT_TEST_DIR,
        plus_num_id=cfg.DATASETS.PLUS_NUM_ID, syn_num=cfg.DATASETS.SYN_NUM, syn_type=cfg.DATASETS.SYN_TYPE)
    ...
    if cfg.MODEL.UDA_STAGE == 'UDA':
        dataset2 = __dataset_dict[cfg.DATASETS.NAMES2](
            cfg, root_train=cfg.DATASETS.ROOT_TRAIN_DIR2, root_val=cfg.DATASETS.ROOT_TEST_DIR,
            plus_num_id=cfg.DATASETS.PLUS_NUM_ID, syn_num=0, syn_type=cfg.DATASETS.SYN_TYPE)
        # dataset2 = __dataset_dict[cfg.DATASETS.NAMES2](
        #     cfg, root_train=cfg.DATASETS.ROOT_TRAIN_DIR2, root_val=cfg.DATASETS.ROOT_TEST_DIR,
        #     plus_num_id=cfg.DATASETS.PLUS_NUM_ID, syn_num=cfg.DATASETS.SYN_NUM, syn_type=cfg.DATASETS.SYN_TYPE)
    ...
    if cfg.MODEL.TASK_TYPE == 'classify_DA':
        if cfg.MODEL.UDA_STAGE == 'UDA':
            return train_loader, train_loader_normal, val_loader, None, num_classes, cam_num, view_num, \
                   train_loader1, train_loader2, img_num1, img_num2, dataset, dataset2
        else:
            return train_loader, train_loader_normal, val_loader, None, num_classes, cam_num, view_num
    else:
...
def make_dataloader_Pseudo(cfg):
    ...
    dataset = __dataset_dict[cfg.DATASETS.NAMES](
        cfg, root=cfg.DATASETS.ROOT_DIR, plus_num_id=cfg.DATASETS.PLUS_NUM_ID,
        syn_num=cfg.DATASETS.SYN_NUM, syn_type=cfg.DATASETS.SYN_TYPE)
