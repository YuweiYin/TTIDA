...
SAVE_DIR="./log/img_clf_cifar100_lt"
mkdir -p "${SAVE_DIR}"
...
function train_img_clf() {
  ...
  epoch=${4}
  seed=${5}
  echo -e "\n\n\n >>> training long_tail ${net} on ${data} [syn_type ${syn_type}] [seed ${seed}]: ${n_img_train} + ${n_img_syn} per class"
  python run_train.py \
    --cuda "${CUDA}" --net "${net}" --data "${data}" --syn_type "${syn_type}" --seed "${seed}" --long_tail \
