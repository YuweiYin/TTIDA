...
from mplug.dataset.caption_dataset import re_train_dataset, re_eval_dataset, pretrain_dataset_4m, \
    coco_dataset, nocaps_dataset
from mplug.dataset.nlvr_dataset import nlvr_dataset
from mplug.dataset.ve_dataset import ve_dataset
from mplug.dataset.vqa_dataset import vqa_dataset
from mplug.dataset.grounding_dataset import build_uni_training_dataset, build_vg_dataset
from mplug.dataset.videoqa_dataset import videoqa_dataset
from mplug.dataset.video_dataset import vatex_video_caps_dataset
from mplug.dataset.randaugment import RandomAugment
...
def create_dataset(dataset, config, epoch=None):
    ...
    if dataset == 'pretrain':
        ...
        train_dataset = coco_dataset(config['train_file'], train_transform, config['coco_root'],
                                     ...
                                     n_img=config["n_img"], n_img_syn=config["n_img_syn"])
        val_dataset = coco_dataset(config['val_file'], test_transform, config['coco_root'],
                                   max_words=config['max_length'], read_local_data=config['read_local_data'],
                                   is_train=False, add_object=config['add_object'],
                                   n_img=config["n_img"], n_img_syn=0)
